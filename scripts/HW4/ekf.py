...
class Ekf(object):
    ...
    def transition_update(self, u, dt):
        ...
        self.x = g
        self.Sigma = Gx.dot(self.Sigma).dot(Gx.T) + dt*Gu.dot(self.R).dot(Gu.T)
...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = np.zeros(2*K)
        Q = np.zeros((2*K, 2*K))
        H = np.zeros((2*K, H_list[0].shape[1]))

        for k in range(K):
            z[2*k: 2*k+2] = v_list[k]
            Q[2*k: 2*k+2, 2*k: 2*k+2] = Q_list[k]
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(z_raw.shape[1]):
            ...
            for j in range(hs.shape[1]):
                v_ij = z_raw[:,i] - hs[:,j]
                v_ij[-1] = angle_diff(z_raw[-1,i], hs[-1,j]) # it seems i can pass tests even w/o this
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        _g, _Gx, _Gu = tb.compute_dynamics(self.x[:3], u, dt)
        g[:3] = _g
        Gx[:3, :3] = _Gx
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            ########## Code starts here ##########
            # TODO: Compute h, Hx.
            h, _Hx = tb.transform_line_to_scanner_frame(np.array([alpha, r]), self.x[:3], self.tf_base_to_camera)
            Hx[:, :3] = _Hx
