...
class TrajectoryTracker:
    ...
    def get_desired_state(self, t):
        ...
        """
        # jjiang play - just to see what happens if for controller we keep on control
    ...
    def compute_control(self, x, y, th, t):
        ...
        V_reset = self.V_prev if self.V_prev>V_PREV_THRES else np.sqrt(xd_d**2+yd_d**2)
        J = np.array([
                [np.cos(th), -V_reset*np.sin(th)],
                [np.sin(th), V_reset*np.cos(th)],
            ])
        Vd, om = np.linalg.solve(J, np.array([u1, u2]))
        V = V_reset + Vd*dt
        ########## Code ends here ##########
