...
class Navigator:
    ...
    def __init__(self):
        ...
        # added by syz,pub a message when a goal is reached
        self.goal_reach_pub = rospy.Publisher('/goal_reach', String, queue_size=10)
        # added by syz,pub a message when path planning fails
        self.goal_cancel_pub = rospy.Publisher('/goal_cancel', String, queue_size=10)
    ...
    def replan(self):
        ...
        if not success:
            ...
            self.goal_cancel_pub.publish("1") # tell the goal_commander that it can move to the next
            return
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.mode == Mode.IDLE:
                ...
                if self.at_goal():
                    # forget about goal:
                    self.goal_reach_pub.publish('1') # tell the goal_commander that it can move to the next
                    self.x_g = None
                    self.y_g = None
                    self.theta_g = None
                    self.switch_mode(Mode.IDLE)
