...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        for i in [-self.resolution, 0, self.resolution]:
            for j in [-self.resolution, 0, self.resolution]:
                ...
                next_neighbor = self.snap_to_grid( (x[0]+i, x[1]+j) )
                if not self.is_free( next_neighbor ): continue
    ...
    def solve(self):
        ...
        while len(self.open_set)>0:
            # TODO: fill me in!
            cur = min(self.open_set, key=lambda x: self.est_cost_through[x])
            if cur == self.x_goal:
            ...
            assert cur in self.cost_to_arrive

            for neighbor in self.get_neighbors(cur):
                if neighbor in self.closed_set: continue

                ###### NOTE @jinglejiang: this distance is NOT self.resolution if we do not view diagonal travelling as only distance 1
                ###### as we think now, due to (1/sqrt(2), 1/sqrt(2)) after snap_to_grid is (1, 1)
                tentative_cost_to_arrive = self.cost_to_arrive[cur] + self.resolution
                ...
                # update known shortest distance
